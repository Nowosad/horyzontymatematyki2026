...
## Kryzys tożsamości geografii {visibility="hidden"}
...
:::
::: {.column width="68%"}
[*fot. Maurycy Żarczyński*]{style="font-size: 70%;"}
```{r}
#| fig-align: center
#| out-width: 100%
...
tm_shape(south_america) +
  tm_borders() +
  tm_shape(splotdata) +
  tm_symbols(fill = "Species_richness", size = 0.5, fill.legend = tm_legend(title = "Bogactwo gatunkowe"))
```
...
Dane w badaniach przyrodniczych są często zaagregowane przestrzennie.
...
tm_shape(pred_rf) +
  tm_raster(col.scale = tm_scale_continuous(values = "spectral", midpoint = 0),
            col.legend = tm_legend(title = "Bogactwo gatunkowe", frame = FALSE)) +
...
Punktem wyjścia jest **macierz współwystępowania** (ang. *co-occurrence matrix*), która opisuje lokalną strukturę sąsiedztwa kategorii.
...
- **Wektor współwystępowania** (ang. *co-occurrence vector*):
```{r}
...
:::: {.columns}
::: {.column width="58%"}
Coraz częściej celem nie jest już tylko analiza pojedynczych zmiennych, lecz tworzenie **wiarygodnych i użytecznych reprezentacji przestrzeni**.

::: {.incremental}
- **Embeddingi i modele fundamentalne (ang. *foundation models*)** próbują tworzyć uniwersalne reprezentacje obrazów satelitarnych oraz danych wieloźródłowych
